...
Agente de IA com RAG (Retrieval-Augmented Generation) que responde, em linguagem
natural, dúvidas sobre as políticas da **BimBam Buy** — um e-commerce fictício —, com
base exclusivamente nos documentos internos da empresa (pagamento, garantia, envio,
devoluções e programa de afiliados). Projeto desenvolvido para o **Challenge Alura
Agente**, do programa [Oracle Next Education (ONE)](docs/descricao_curso.md).
...
flowchart LR
    ...
    Pergunta["Pergunta do usuário"] --> Retrieval["Busca por similaridade\n(top-k)"]
    FAISS --> Retrieval
...
fica só em `st.session_state`, nunca é salva em arquivo, banco ou log.
...
```
app.py                      # interface Streamlit (chat + modal de configuração)
src/
  config.py                 # system prompt, mensagem de recusa, limiares, provedores
  ...
  rag_agent.py              # guardrail de similaridade + montagem do prompt + chamada ao LLM
...
O app abre em `http://localhost:8501`. Ao abrir, clique na engrenagem (⚙️) no topo,
escolha o provedor, cole sua chave de API e clique em **Salvar** — os 5 documentos da
BimBam Buy são indexados automaticamente nesse momento. Depois é só perguntar no chat.
Opcionalmente, na mesma janela de configuração, é possível anexar PDFs ou CSVs extras
para a sessão (não são salvos em disco, somem ao fechar a aba).
...
- [ ] Validação das respostas reais do agente com uma chave de API (substituir as
      respostas de referência acima por transcrições reais)
- [ ] Deploy na nuvem (Oracle Cloud Infrastructure ou alternativa)
